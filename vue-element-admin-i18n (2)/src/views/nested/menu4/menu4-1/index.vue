...
<!-- 	table: {
      ...
      <el-table-column :label="$t('ID')"  prop="id"  align="center"  min-width="6%" :class-name="getSortClass('id')" >
       ...
       <el-table-column :label="$t('table.productName')" min-width="15%" align="center">
             <template slot-scope="{row}">
        ...
        <el-table-column :label="$t('table.evaluationNumber')" min-width="13%" align="center">
             <template slot-scope="{row}">
               <span>{{ row.evaluationNumber }}</span>
             </template>
           </el-table-column>
       
        <el-table-column :label="$t('table.shopName')" min-width="16%" align="center">
             <template slot-scope="{row}">
               <span>{{ row.shopName }}</span>
             </template>
           </el-table-column>
        <el-table-column :label="$t('table.commodityLink')" min-width="20%" align="center">
             <template slot-scope="{row}">
               <span @click="dump(row.commodityLink)">{{ row.commodityLink }}</span>
             </template>
           </el-table-column>
        <el-table-column :label="$t('table.imageLink')" min-width="20%" align="center">
             <template slot-scope="{row}">
               <span  @click="dump(row.imageLink)">{{ row.imageLink }}</span>
             </template>
           </el-table-column>
...
<script>
import { fetchList } from '@/api/snyg'
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['ID', '商品名称', '价格', '评论数', '店铺名称', '商品链接', '图片链接']
        const filterVal = ['id', 'productName', 'price', 'evaluationNumber', 'shopName', 'commodityLink', 'imageLink']
