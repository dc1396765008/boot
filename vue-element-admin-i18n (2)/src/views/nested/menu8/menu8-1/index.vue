...
<!-- 	table: {
      ...
      <el-table-column :label="$t('ID')"  prop="id"  align="center"  min-width="6%" :class-name="getSortClass('id')" >
       ...
       <el-table-column :label="$t('table.name')" min-width="12%" align="center">
             <template slot-scope="{row}">
               <span>{{ row.name }}</span>
             </template>
           </el-table-column>
		   <el-table-column :label="$t('table.shopName')" min-width="12%" align="center">
		         <template slot-scope="{row}">
		           <span>{{ row.shopName }}</span>
		         </template>
		       </el-table-column>
       
        <el-table-column :label="$t('table.url')" min-width="20%" align="center">
             <template slot-scope="{row}">
               <span @click="dump(row.url)">{{ row.url }}</span>
             </template>
           </el-table-column>
       <el-table-column :label="$t('table.imgUrl')" min-width="20%" align="center">
            <template slot-scope="{row}">
        ...
        <el-table-column :label="$t('table.address')" min-width="20%" align="center">
             <template slot-scope="{row}">
  ...
  methods: {
    ...
    handleDownload() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['ID', '商品名称', '店铺名称','商品链接', '图片链接', '价格','地址']
        const filterVal = ['id', 'name', 'shopName', 'url','imgUrl', 'price','address']
